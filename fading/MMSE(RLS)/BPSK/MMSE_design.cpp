...
const float lamda = 0.975;								// Weighting factor of Kalman Filter (0<lamda<1)
...
//float CIR[3] = {(1+cos(-2*Pi/2.9))/2.0, (1+cos(0.0))/2.0, (1+cos(2*Pi/2.9))/2.0};
const float vc = 60.0; 								/* speed of vehicle in km/hr */
...
{
	time_t  t, start, end;
	int i, j, l, p, *data_bit, *ch, from_state, tmp_state;
   double out_sym_I, out_sym_Q, **W, **U, Output[2], *MSE, ***P;
   double snr, Eb_No, noise_pwr, noise[2], **Yk, e[2], **Kalman;
